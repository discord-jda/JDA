...
//		playBack(format, decodedFromNetwork);
	...
	private ShortBuffer decode(List<ByteBuffer> packets) {
		...
		PointerByReference opusDecoder = Opus.INSTANCE.opus_decoder_create(48000, 1, error);
	...
	private List<ByteBuffer> encode(ShortBuffer shortBuffer) {
		IntBuffer error = IntBuffer.allocate(4);
		PointerByReference opusEncoder = Opus.INSTANCE.opus_encoder_create(48000, 1, Opus.OPUS_APPLICATION_RESTRICTED_LOWDELAY, error);
		int read = 0;
