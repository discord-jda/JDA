...
//				File f = Native.extractFromResourcePath("opus");
	...
	public void testCodec() throws LineUnavailableException {
        ...
        System.out.println("Attempting to record");
		ShortBuffer dataFromMic = recordFromMicrophone(format, 5000);
        System.out.println("Recording complete");
        System.out.println("Now.. to encode.");
...
//		{
//			byte[] bytes = new byte[packet.array().length + 12];
//			System.arraycopy(packet.array(), 0, bytes, 12, packet.array().length);
//			packet = ByteBuffer.wrap(bytes);
////			AudioWebSocket.INSTANCE.convertBuffer(packet);
//			packet.putInt(4, time);
//			time += 8000;
//			DatagramPacket sendPacket = new DatagramPacket(packet.array(), packet.array().length, AudioWebSocket.INSTANCE.address);
//			try
//			{
//				AudioWebSocket.INSTANCE.udpSocket.send(sendPacket);
//			}
//			catch (IOException e)
//			{
//				e.printStackTrace();
...
//		playBack(format, decodedFromNetwork);
	...
	private List<ByteBuffer> encode(ShortBuffer shortBuffer) {
		IntBuffer error = IntBuffer.allocate(4);
		PointerByReference opusEncoder = Opus.INSTANCE.opus_encoder_create(RATE, 2, Opus.OPUS_APPLICATION_VOIP, error);
		int read = 0;
