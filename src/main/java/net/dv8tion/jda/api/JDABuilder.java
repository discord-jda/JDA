...
/**
 * Used to create new {@link net.dv8tion.jda.api.JDA} instances. This is also
 * useful for making sure all of your
 * {@link net.dv8tion.jda.api.hooks.EventListener EventListeners} are registered
 * before {@link net.dv8tion.jda.api.JDA} attempts to log in.
 *
 * <p>
 * A single JDABuilder can be reused multiple times. Each call to
 * {@link net.dv8tion.jda.api.JDABuilder#build() build()} creates a new
 * {@link net.dv8tion.jda.api.JDA} instance using the same information. This
 * means that you can have listeners easily registered to multiple
 * {@link net.dv8tion.jda.api.JDA} instances.
 */
public class JDABuilder {
	public static final int GUILD_SUBSCRIPTIONS = GatewayIntent.getRaw(GatewayIntent.GUILD_MEMBERS,
	...
	/**
	 * Creates a JDABuilder with recommended default settings. <br>
	 ...
	 * enable additional intents with {@link #enableIntents(Collection)}.
	 *
	 * <p>
	 * If you don't enable certain intents, the cache will be disabled. For
	 * instance, if the {@link GatewayIntent#GUILD_MEMBERS GUILD_MEMBERS} intent is
	 * disabled, then members will only be cached when a voice state is available.
	 * If both {@link GatewayIntent#GUILD_MEMBERS GUILD_MEMBERS} and
	 * {@link GatewayIntent#GUILD_VOICE_STATES GUILD_VOICE_STATES} are disabled then
	 * no members will be cached.
	 *
	 * <p>
	 * The individual {@link CacheFlag CacheFlags} will also be disabled if the
	 * {@link CacheFlag#getRequiredIntent() required intent} is not enabled.
	 *
	 * @param token   The bot token to use
	 * @param intent  The intent to enable
	 * @param intents Any other intents to enable
	 *
	 * @throws IllegalArgumentException If provided with null intents
	 *
	 * @return The new JDABuilder
	 */
	@Nonnull
	@CheckReturnValue
	public static JDABuilder createDefault(@Nullable String token, @Nonnull GatewayIntent intent,
	...
	private JDABuilder applyDefault() {
		return this.setMemberCachePolicy(MemberCachePolicy.DEFAULT).setChunkingFilter(ChunkingFilter.NONE)
				.disableCache(CacheFlag.getPrivileged()).setLargeThreshold(250);
	}

	/**
	 * Creates a JDABuilder with low memory profile settings. <br>
	 * Note that these defaults can potentially change in the future.
	 *
	 * <ul>
	 * <li>{@link #setEnabledIntents(Collection)} is set to
	...
	@Nonnull
	@CheckReturnValue
	public static JDABuilder createLight(@Nullable String token, @Nonnull GatewayIntent intent,
			@Nonnull GatewayIntent... intents) {
		Checks.notNull(intent, "GatewayIntent");
		Checks.noneNull(intents, "GatewayIntent");
	...
	private JDABuilder applyLight() {
		return this.setMemberCachePolicy(MemberCachePolicy.NONE).setChunkingFilter(ChunkingFilter.NONE)
				.disableCache(EnumSet.allOf(CacheFlag.class)).setLargeThreshold(50);
	}

	/**
	 * Creates a completely empty JDABuilder with the predefined intents. <br>
	 ...
	 * @param intents The gateway intents to use
	 *
	 * @throws IllegalArgumentException If the provided intents are null
	 *
	...
	/**
	 * Creates a JDABuilder with the predefined token. <br>
	 ...
	 *
	 * @param token   The bot token to use
	 * @param intent  The first gateway intent to use
	 * @param intents Additional gateway intents to use
	 *
	 * @throws IllegalArgumentException If the provided intents are null
	 *
	 * @return The JDABuilder instance
	...
	@Nonnull
	@CheckReturnValue
	public static JDABuilder create(@Nullable String token, @Nonnull GatewayIntent intent,
			@Nonnull GatewayIntent... intents) {
	 ...
	 * Default: <b>true</b>
	 *
	 * @param enable True, if the relative {@code X-RateLimit-Reset-After} header
	 *               should be used.
	 *
	 * @return The JDABuilder instance. Useful for chaining.
	 *
	 ...
	 *
	 * @param map The <b>modifiable</b> context map to use in JDA, or {@code null}
	 *            to reset
	 *
	 * @return The JDABuilder instance. Useful for chaining.
	 *
	 ...
	 * switch to stay.
	 *
	 * @param compression The compression algorithm to use with the gateway
	 *                    connection
	 *
	...
	/**
	 * Sets the token that will be used by the {@link net.dv8tion.jda.api.JDA}
	 ...
	 *
	 * @param pool              The thread-pool to use for rate-limit handling
	 * @param automaticShutdown Whether {@link JDA#shutdown()} should shutdown this
	 *                          pool
	 *
	 * @return The JDABuilder instance. Useful for chaining.
	 */
	 ...
	 * lifetime sleeping because they only activate for sending messages over the
	 * gateway. <br>
	 ...
	 * can enable it using {@link #setGatewayPool(ScheduledExecutorService, boolean)
	 ...
	 *
	 * @param pool The thread-pool to use for WebSocket workers
	 *
	 * @return The JDABuilder instance. Useful for chaining.
	 */
	@Nonnull
	 ...
	 *
	 * <p>
	 * The executor will not be shutdown automatically when JDA is shutdown. To shut
	 * it down automatically use {@link #setEventPool(ExecutorService, boolean)}.
	 *
	 ...
	 * Default: <b>true (enabled)</b>
	 *
	 * @param enable True (default) - use shutdown hook to clean up JDA if the Java
	 *               program is closed.
	 *
	 * @return Return the {@link net.dv8tion.jda.api.JDABuilder JDABuilder }
	 ...
	 *
	 * @param manager The new {@link net.dv8tion.jda.api.hooks.IEventManager} to
	 *                use.
	 *
	 * @return The JDABuilder instance. Useful for chaining.
	 */
	 ...
	 *
	 * @throws java.lang.IllegalArgumentException If the provided shard
	 ...
	 * {@link #setMemberCachePolicy(MemberCachePolicy)} will be ignored.
	 *
	 ...
	 * flag are documented in the {@link CacheFlag} enum.
	 *
	 * @param intents The intents to disable (default: none)
	 *
	 * @return The JDABuilder instance. Useful for chaining.
	 *
	 * @see #setMemberCachePolicy(MemberCachePolicy)
	 ...
	 * flag are documented in the {@link CacheFlag} enum.
	 *
	 * @param intents The intents to enable (default: all)
	 *
	 * @return The JDABuilder instance. Useful for chaining.
	 *
	 * @see #setMemberCachePolicy(MemberCachePolicy)
	...
	/**
	 * Builds a new {@link net.dv8tion.jda.api.JDA} instance and uses the provided
	 ...
	 * empty list or {@link net.dv8tion.jda.api.JDA#getUserById(long)} might return
	 * null for arbitrary user IDs.
	 *
	 * <p>
	 ...
	 * @throws IllegalArgumentException If the provided token is empty or null. Or
	 *                                  the provided intents/cache configuration is
	 *                                  not possible.
	 *
	 * @return A {@link net.dv8tion.jda.api.JDA} instance that has started the login
	 *         process. It is unknown as to whether or not loading has finished when
	 *         this returns.
	 *
	...
	public JDA build() throws LoginException {
		...
		SessionConfig sessionConfig = new SessionConfig(controller, httpClient, wsFactory, voiceDispatchInterceptor,
				flags, maxReconnectDelay, largeThreshold);
		...
		jda.setStatus(JDA.Status.INITIALIZED); // This is already set by JDA internally, but this is to make sure the
												// listeners catch it.

		// Set the presence information before connecting to have the correct
		// information ready when sending IDENTIFY
		((PresenceImpl) jda.getPresence()).setCacheActivity(activity).setCacheStatus(status);
