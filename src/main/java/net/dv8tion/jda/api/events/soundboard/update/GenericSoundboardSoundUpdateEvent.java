 ...
 * which requires {@link GatewayIntent#GUILD_EXPRESSIONS}.
 *
 * <br>{@link JDABuilder#createLight(String) createLight(String)} disables that CacheFlag by default!
 */
public abstract class GenericSoundboardSoundUpdateEvent<T> extends GenericSoundboardSoundEvent
        implements UpdateEvent<SoundboardSound, T> {
    ...
    public GenericSoundboardSoundUpdateEvent(
            @Nonnull JDA api,
            long responseNumber,
            @Nonnull SoundboardSound soundboardSound,
            @Nullable T previous,
            @Nullable T next,
