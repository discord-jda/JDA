...
/**
 * Indicates that the [Privileges][IntegrationPrivilege] of an application-command on a guild changed.
 * <br></br>If the moderator updates application-wide privileges instead of command, a [ApplicationUpdatePrivilegesEvent] will be fired instead.
 *
 *
...
class ApplicationCommandUpdatePrivilegesEvent(
    ...
    targetId: Long, applicationId: Long, @Nonnull privileges: List<IntegrationPrivilege>?
) : GenericPrivilegeUpdateEvent(api, responseNumber, guild, targetId, applicationId, privileges) {
    @get:Nonnull
    override val targetType: PrivilegeTargetType?
        get() = PrivilegeTargetType.COMMAND
