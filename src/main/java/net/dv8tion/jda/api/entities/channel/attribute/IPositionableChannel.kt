 ...
 */
interface IPositionableChannel : GuildChannel {
    @get:Nonnull
    abstract override val manager: ChannelManager<*, *>?
    val position: Int
        /**
         * The position of this channel in the channel list of the guild.
         * <br></br>This does not account for thread channels, as they do not have positions.
         *
         *
         * This is functionally equivalent to `getGuild().getChannels().indexOf(channel)`.
         * To efficiently compare the position between channels, it is recommended to use [.compareTo] instead of the position.
         *
         ...
         */
        get() {
            val position = getGuild().channels.indexOf(this)
            if (position > -1) return position
            throw IllegalStateException("Somehow when determining position we never found the " + getType().name + " in the Guild's channels? wtf?")
        }
     ...
     *
     * @return The true, Discord stored, position of the [GuildChannel].
     */
    @JvmField
    val positionRaw: Int
}
