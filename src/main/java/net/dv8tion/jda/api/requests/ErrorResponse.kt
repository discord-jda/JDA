...
/**
 * Constants for easy use in [ErrorResponseException][net.dv8tion.jda.api.exceptions.ErrorResponseException] and [ErrorHandler][net.dv8tion.jda.api.exceptions.ErrorHandler].
 *
 * @see RestAction
 *
 * @see net.dv8tion.jda.api.exceptions.ErrorHandler ErrorHandler
 *
 * @see [Discord Error Codes](https://discord.com/developers/docs/topics/opcodes-and-status-codes.json)
 */
enum class ErrorResponse(val code: Int, @get:Nonnull val meaning: String) {
     ...
     * <br></br>This is very useful in combination with [RestAction.onErrorMap] and [RestAction.onErrorFlatMap]!
     *
     * @param  throwable
     ...
     */
    fun test(throwable: Throwable?): Boolean {
        return throwable is ErrorResponseException && throwable.errorResponse == this
    }
    ...
    companion object {
         ...
         *
         * @return [Predicate] which returns true, if the error response is equal to this
         */
        @Nonnull
        fun test(@Nonnull vararg responses: ErrorResponse?): Predicate<Throwable> {
            Checks.noneNull(responses, "ErrorResponse")
            val set = EnumSet.noneOf(ErrorResponse::class.java)
         ...
         */
        @Nonnull
        fun test(@Nonnull responses: Collection<ErrorResponse?>?): Predicate<Throwable> {
            Checks.noneNull(responses, "ErrorResponse")
            val set = EnumSet.copyOf(responses)
            return test(set)
        }

        private fun test(@Nonnull responses: EnumSet<ErrorResponse?>): Predicate<Throwable> {
            return Predicate { error: Throwable? ->
                error is ErrorResponseException && responses.contains(
                    error.errorResponse
                )
            }
        }
        ...
        @Nonnull
        fun fromCode(code: Int): ErrorResponse {
        ...
        fun fromJSON(obj: DataObject?): ErrorResponse {
            return if (obj == null || obj.isNull("code")) SERVER_ERROR else fromCode(
                obj.getInt("code")
            )
        }
    }
