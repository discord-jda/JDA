 ...
 *
 * **Example**
 * <pre>`manager.setName("no-more-news")
 * .setType(ChannelType.TEXT) // Changes channel type to TextChannel
 * .queue();
`</pre> *
 */
interface NewsChannelManager : StandardGuildMessageChannelManager<NewsChannel?, NewsChannelManager?> {
    /**
     ...
     * If this ChannelAction is not for a [TextChannel] or [NewsChannel]
    ...
    @Nonnull
    @CheckReturnValue
    fun setType(@Nonnull type: ChannelType?): NewsChannelManager?
}
