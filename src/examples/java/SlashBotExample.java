...
public class SlashBotExample extends ListenerAdapter
{
    ...
    public void ban(SlashCommandInteractionEvent event, User user, Member member)
    {
        ...
        Member selfMember = event.getGuild().selfMember;
        ...
        String reason = event.getOption("reason",
                () -> "Banned by " + event.getUser().name, // used if getOption("reason") is null (not provided)
        ...
        event.getGuild().ban(user, delDays, TimeUnit.DAYS)
            ...
            .flatMap(v -> hook.sendMessage("Banned user " + user.name)) // chain a followup message after the ban is executed
